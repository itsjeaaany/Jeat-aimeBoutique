<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-[#d76f49] leading-tight">{{ __('Edit Product') }}</h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gradient-to-br from-[#2c3e50] to-[#34495e] overflow-hidden shadow-lg rounded-2xl p-8">
                <h1 class="text-2xl font-bold text-black mb-6">{{ __('Edit Product') }}</h1>
                <form action="{{ route('seller.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="lg:col-span-1">
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full bg-[#ffe7dd] text-gray-900 placeholder-[#6b7280] rounded-lg border-0 focus:ring-2 focus:ring-[#d76f49]" placeholder="Enter product name" value="{{ old('name', $product->name) }}" required />
                        ...
                        <div class="lg:col-span-2">
                            ...
                            @if($product->image)
                                <div class="mb-3 p-3 bg-[#1a252f] rounded-lg">
                                    <p class="text-sm text-gray-400 mb-2">Current photo:</p>
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="h-24 w-24 object-cover rounded-lg">
                                </div>
                            @endif
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" class="mt-1 block w-full bg-[#ffe7dd] text-black placeholder-[#6b7280] rounded-lg border border-[#d76f49]/30 focus:ring-2 focus:ring-[#d76f49] focus:border-transparent" placeholder="Describe your product..." required>{{ old('description', $product->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <select id="category" name="category" class="mt-1 block w-full bg-[#ffe7dd] text-gray-900 rounded-lg border border-[#d76f49]/30 focus:ring-2 focus:ring-[#d76f49] focus:border-transparent" required>
                            ...
                            <option value="Beauty & Personal Care" {{ old('category', $product->category) === 'Beauty & Personal Care' ? 'selected' : '' }}>Beauty & Personal Care</option>
                            <option value="Home Decor & Gifts" {{ old('category', $product->category) === 'Home Decor & Gifts' ? 'selected' : '' }}>Home Decor & Gifts</option>
                            <option value="Unique Finds" {{ old('category', $product->category) === 'Unique Finds' ? 'selected' : '' }}>Unique Finds</option>
                        </select>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                            @foreach(['XS', 'S', 'M', 'L', 'XL'] as $size)
                                <label class="flex items-center p-3 bg-[#ffe7dd] rounded-lg border border-[#d76f49]/30 cursor-pointer hover:bg-[#ffd4c4] transition">
                                    <input type="checkbox" name="sizes[]" value="{{ $size }}" class="rounded border-[#d76f49] text-[#d76f49] focus:ring-[#d76f49]" {{ (in_array($size, old('sizes', $product->sizes ?? []))) ? 'checked' : '' }} />
                    ...
                    <div id="footwearSizes" class="hidden">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-8 gap-3">
                            @foreach([36, 37, 38, 39, 40, 41, 42, 43] as $size)
                                <label class="flex items-center p-3 bg-[#ffe7dd] rounded-lg border border-[#d76f49]/30 cursor-pointer hover:bg-[#ffd4c4] transition">
                                    <input type="checkbox" name="sizes[]" value="{{ $size }}" class="rounded border-[#d76f49] text-[#d76f49] focus:ring-[#d76f49]" {{ (in_array((string)$size, old('sizes', $product->sizes ?? []))) ? 'checked' : '' }} />
                                    <span class="ml-2 text-gray-900 font-medium">{{ $size }}</span>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="price" name="price" type="number" step="0.01" class="mt-1 block w-full bg-[#ffe7dd] text-gray-900 placeholder-[#6b7280] rounded-lg border-0 focus:ring-2 focus:ring-[#d76f49]" placeholder="0.00" value="{{ old('price', $product->price) }}" required />
                            <x-input-error :messages="$errors->get('price')" class="mt-2" />
                        </div>
                        <div>
                            <label for="stock" class="block text-sm font-medium text-black mb-2">{{ __('Stock Quantity') }}</label>
                            <x-text-input id="stock" name="stock" type="number" class="mt-1 block w-full bg-[#ffe7dd] text-gray-900 placeholder-[#6b7280] rounded-lg border-0 focus:ring-2 focus:ring-[#d76f49]" placeholder="0" value="{{ old('stock', $product->stock) }}" required />
